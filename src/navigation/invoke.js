...
export function invoke(pendings, eventArgs) {
    ...
    function performAppChanges() {
        ...
        let loadPromises = loadApps.map(app => {
            return toLoadPromise(app).then(app => {
                // 备注: 在 unmountPromises.then 之后执行
                // 1 先卸载
                // 2 把 callAllLocationEvents 把 popevent hashchangeevent 事件回调执行完
                // 3 load app
                // 4 mound app
                console.log('toLoadPromise run', app);
    ...
    function finish() {
        console.log('loadAndMountPromises end');
    ...
    function callAllLocationEvents() {
        ...
        console.log('callAllLocationEvents run =================================');
